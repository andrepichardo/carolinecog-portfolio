...
quiet(`npx prisma dev stop ${NAME}`);
await sleep(1500);
...
const startOutput = strip(quiet(`npx prisma dev --name ${NAME} --detach`));
...
while (Date.now() < deadline && !running) {
  await sleep(1000);
  const listing = strip(quiet('npx prisma dev ls'));
...
console.log('  ' + strip(quiet('npx prisma db push --skip-generate')).trim().split('\n').slice(-1)[0]);

console.log('· importando el contenido…');
console.log(
  strip(quiet('npx tsx scripts/import-readymag.ts'))
    .split('\n')
...
console.log(
  strip(quiet('npx tsx prisma/seed.ts'))
    .split('\n')
